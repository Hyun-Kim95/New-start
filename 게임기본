...
charactor_height = charactor_size[1] #캐릭터의 세로크기
charactor_x_pos = (screen_width / 2) - (charactor_width / 2) #화면가로의 절반 크기에 해당하는 곳에 위치
charactor_y_pos = screen_height - charactor_height #화면세로의 가장 아래에 해당하는 곳에 위치
...
enemy_x_pos = (screen_width / 2) - (enemy_width / 2) #화면가로의 절반 크기에 해당하는 곳에 위치
enemy_y_pos = (screen_height / 2) - (enemy_height / 2) #화면세로의 가장 아래에 해당하는 곳에 위치
...
while running:
    ...
    screen.blit(background, (0,0)) #0,0위치에 배경 그려줌
    screen.blit(charactor, (charactor_x_pos, charactor_y_pos)) #캐릭터그리기
