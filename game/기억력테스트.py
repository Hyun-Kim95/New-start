...
def shuffle_grid(number_count):
    ...
    button_size = 110   # grid cell 내에 실제로 그려질 버틑 크기
    ...
    number = 1 # 시작 숫자 1부터 number_count 까지 랜덤으로 배치
...
def display_start_screen():
    ...
    # screen 에 흰색으로 동그라미를 그리는데 중심 좌표는 start_button.center,
    # 반지름 60, 선 두깨는 5
...
def check_number_buttons(pos):
    ...
    for button in number_buttons:
        if button.collidepoint(pos):
            if button == number_buttons[0]: # 올바를 숫자 클릭
...
# 시작 버튼
start_button = pygame.Rect(0,0,120,120) # 가로 세로 120인 사각형
start_button.center = (120,screen_height-120)   # 사각형의 중심위치 설정
