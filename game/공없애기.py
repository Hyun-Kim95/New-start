...
balls.append({
    ...
    "to_x": 5, # x축 이동방향, -3 이면 왼쪽으로, 3 이면 오른쪽으로
    "to_y": -8, # y축 이동방향, 
...
while running:
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                weapon_to_remov = weapon_idx # 해당무기 없애기 위한 값 설정
                ball_to_remov = ball_idx #해당 공 없애기 위한 값 설정
    ...
    # 충돌된 공 or 무기 없애기
    if ball_to_remov > -1:
        del balls[ball_to_remov]
        ball_to_remov = -1

    if weapon_to_remov > -1:
        del weapons[weapon_to_remov]
        weapon_to_remov = -1
